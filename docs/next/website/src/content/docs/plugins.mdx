...
```bash
herdr plugin install OnlineChefGroep/herdr-plugin-examples/agent-telegram-notify
...
The example cookbook repo is `OnlineChefGroep/herdr-plugin-examples`. It contains
...
## CHEF fleet plugins (preview)

CHEF fleet plugins under `com.chefgroep.*` contribute optional personal context
to the Fleet Ops Bar by writing `fleet_ops.json` into
`$HERDR_PLUGIN_STATE_DIR`. Link a local scaffold with
`herdr plugin link <path>`, then invoke actions through
`herdr plugin action invoke <action-id> --plugin <plugin-id>`.

Linear remains the source of truth for work items; GitHub for PR/CI; host and
Cloudflare APIs for runtime health. Do not store secrets in `fleet_ops.json`.
Toggle the bar in Settings → fleet (`ui.fleet_ops_bar`).
