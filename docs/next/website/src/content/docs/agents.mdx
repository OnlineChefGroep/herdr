...
Bundled manifests live inside Herdr. Herdr also checks herdr.chefgroep.nl for remote manifest updates and applies valid per-agent rule updates automatically without requiring a Herdr restart. Remote manifests are stored in Herdr's state directory. Set `[update] manifest_check = false` to disable background remote manifest checks.
