...
The Cursor Cloud VM snapshot already has the required toolchains on `PATH`: Rust 1.96.1 (auto-selected via `rust-toolchain.toml`), `zig` 0.15.2 at `/usr/local/bin/zig` (mandatory — `build.rs` runs `zig build` to compile the vendored `libghostty-vt`; without it no build works), plus `just`, `cargo-nextest`, and `bun`. The startup update script only refreshes dependencies (`cargo fetch`, `bun install` for `workers/plugin-marketplace`); it does not install these toolchains.

Standard workflow commands are already documented above (Testing section) and in `justfile`: `just lint`, `just test`, `just check`, `cargo build`.
...
- Running the source build from a plain shell (not inside a Herdr session) auto-spawns a debug server in the separate `herdr-dev` namespace (socket `~/.config/herdr-dev/herdr.sock`), so it never touches an installed stable server. As noted above, clear `HERDR_SOCKET_PATH`/`HERDR_CLIENT_SOCKET_PATH` when running from source.
- The TUI needs a real terminal (TTY). For headless verification, run `./target/debug/herdr server` and drive it with the CLI/socket API (`herdr workspace create`, `herdr pane run <pane_id> <cmd>`, `herdr pane read <pane_id>`) — this exercises real panes without a TTY.
- Baseline test status on this fork (`main`): the Rust suite has 5 pre-existing failures unrelated to environment setup — `live_handoff::live_server_holds_one_pty_master_fd_per_pane` (inspects `/dev/ptmx` fds, sandbox-sensitive), `client::tests::graphics_bytes_are_written_after_blit_with_saved_cursor`, two `server::headless::tests` keybinding tests, and `ui::panes::tests::pane_border_renderer_places_adjacent_cjk_by_display_width` (a byte-slice-on-non-char-boundary panic in fork-specific `FleetOpsMetadata` bar code). Treat these as the known baseline, not regressions you introduced. All Python (60) and Bun (15) tests pass.
