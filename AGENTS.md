...
- The full suite passes here (2659 Rust nextest, 60 Python, 15 Bun). Note the Linux `live_handoff::live_server_holds_one_pty_master_fd_per_pane` test inspects `/proc/<pid>/fd` for PTY master fds and accepts both `/dev/ptmx` and `/dev/pts/ptmx` (in this container `/dev/ptmx` is a symlink to `/dev/pts/ptmx`, so opened master fds canonicalize to the latter).
