...
// `oldest_hours` on `PluginParkedSummary` is `f64`, so this fragment (and the
// parked summary it embeds) can only derive `PartialEq`, not `Eq`.
...
// `oldest_hours` is `f64` (session-park writes fractional hours), so this
// struct can only derive `PartialEq`, not `Eq`.
#[derive(Debug, Clone, Default, Deserialize, PartialEq)]
